    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input, float facing : VFACE) : SV_Target
            {
                ...
                float3 finalNormal = lerp(nMain, lerp(nUV3, nUV2, uv2Alpha), uvAlpha);
                float3 N = normalize(mul(finalNormal, TBN));
                ...
                float3 diffuse = baseCol * nl * mainLight.color;
                finalColor += diffuse + specCol * mainLight.distanceAttenuation;
                ...
                for (int i = 0; i < addLightCount; i++)
                {
                    ...
                    float3 diffAdd = baseCol * nl_add * light.color * light.distanceAttenuation;
                    finalColor += diffAdd + specCol_add * light.distanceAttenuation;
                }

                // 菲涅尔
                half fresnel = _FresnelScale * pow(1.0 - saturate(dot(normalWS, viewDirWS)), _FresnelRange);
                finalColor = lerp(finalColor, finalColor * _FresnelColor.rgb, fresnel);
        ...
        Pass
        {
            ...
            {
                Varyings output;
                output.positionHCS = TransformWorldToHClip(ApplyShadowBias(input.positionOS.xyz, float3(0, 1, 0), 0.0));
                return output;
            }
