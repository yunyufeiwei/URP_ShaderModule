...
public class Rope : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        allMass = new Mass02[massCount];
        //创建所有质点
        for (int i = 0; i < massCount; i++)
        {
            var item = GameObject.CreatePrimitive(PrimitiveType.Sphere).AddComponent<Mass02>();
        ...
        // 创建所有弹簧 
        allSprings = new List<Spring02>();
        for (int i = 0; i < massCount - 1; i++)
        {
            var sp = allMass[i].gameObject.AddComponent<Spring02>();
            sp.mass_a = sp.GetComponent<Mass02>();
            sp.mass_b = allMass[i + 1].GetComponent<Mass02>();
            allSprings.Add(sp);
        }
