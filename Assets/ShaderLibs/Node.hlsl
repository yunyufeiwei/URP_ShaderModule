...
// ================================= Flow Map =================================
half4 FlowMapNode(Texture2D mainTex, SamplerState sampler_mainTex, Texture2D flowMap, SamplerState sampler_flowMap, float2 mainUV, float tilling, float flowSpeed, float strength)
{
...
float3 BlendNormalRNMOverride(float3 n1, float3 n2)
{
...
// ================================= 分层Diffuse =================================
half3 ColorLayer(float NdotL, half smoothness, half threshold1, half threshold2, half3 color1, half3 color2, half3 color3)
{
...
// ================================ 横向或纵向溶解 ================================
half4 DissolveLinear(half4 color, sampler2D NoiseTex, float2 uv,half threshold, half noiseIntensity, half edgeLength, half edgeBlur,half4 edgeFirstColor, half4 edgeSecondColor)
{
...
// ================================= 根据深度重建世界坐标 =================================
float4 CalculateWorldPosition(float2 screen_uv, float eyeDepth)
{
    // NDC
...
// ================================= 压暗对比色 =================================
// todo 测试...
float3 ToneMaping(float3 color)
{
...
// ================================ 转换亮度值 ================================
float LuminanceOverride(float3 rgb)
{
    return dot(rgb, float3(0.2126729, 0.7151522, 0.0721750));
}
// ================================= 三维映射 =================================
half4 TriplanarMapping(Texture2D textures , SamplerState sampler_Texture , float3 positionWS, half3 N, float tiling, float blendSmoothness)
{
    half2 yUV = positionWS.xz * tiling;
    half2 xUV = positionWS.zy * tiling;
    half2 zUV = positionWS.xy * tiling;

    half3 yDiff = SAMPLE_TEXTURE2D(textures , sampler_Texture , yUV).xyz;
    half3 xDiff = SAMPLE_TEXTURE2D(textures , sampler_Texture , xUV).xyz;
    half3 zDiff = SAMPLE_TEXTURE2D(textures , sampler_Texture , zUV).xyz;
